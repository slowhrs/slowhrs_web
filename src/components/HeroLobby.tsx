...
function posterBg(posterSrc: string, visible: boolean): CSSProperties {
  if (!visible) return { backgroundColor: "#050505" };
  return {
...
export default function HeroLobby() {
  ...
  const [layerPlaying, setLayerPlaying] = useState<Record<number, boolean>>({});
  const timerRef = useRef<NodeJS.Timeout | null>(null);
  const videoRefs = useRef<Array<HTMLVideoElement | null>>([]);

  const markLayerPlaying = useCallback((index: number) => {
    setLayerPlaying((prev) => (prev[index] ? prev : { ...prev, [index]: true }));
  }, []);
  ...
  useLayoutEffect(() => {
    ...
    playAll();
    const playTimer = window.setInterval(playAll, 400);
  ...
  const renderVideoLayer = (src: string, index: number) => {
    ...
    const className =
      "absolute inset-0 w-full h-full object-cover bg-cover bg-center filter brightness-[0.65] contrast-[1.05] saturate-[1.1] transition-opacity duration-[1200ms] ease-in-out";
    const showPosterBg = isActive
      ? index === 0
        ? !recapPlaying
        : !layerPlaying[index]
      : false;
    const style: CSSProperties = {
      ...
      ...posterBg(posterSrc, showPosterBg),
    };

    const bindRef = (node: HTMLVideoElement | null) => {
    ...
    if (src === HERO_RECAP_VIDEO) {
      return (
        <HeroVideo
          ...
          onPlayingChange={setRecapPlaying}
        />
      );
    }
  ...
  const wrapperPosterVisible =
    activeIdx === 0 ? !recapPlaying : !layerPlaying[activeIdx];
  ...
  return (
    <section className="relative min-h-[calc(100vh-26px)] overflow-hidden grid grid-rows-[1fr_auto] p-0">
      <div
        ...
        style={{
          ...posterBg(posterFor(HERO_VIDEOS[activeIdx]), wrapperPosterVisible),
        }}
      >
          ...
          >
            <div
              className={`absolute inset-0 transition-colors duration-300 ${
                i === activeIdx
                  ? "bg-brand-red shadow-[0_0_6px_var(--red)]"
                  : "bg-brand-ink/25 group-hover:bg-brand-ink/50"
              }`}
            />
          </button>
        ))}
      ...
      <div className="relative z-[3] px-6 pt-[9rem] pb-12 md:px-12 md:pt-[11rem] flex flex-col justify-end min-h-[calc(100vh-26px-80px)]">
        ...
        <div className="flex flex-col items-start gap-4 font-mono text-[11px] tracking-[0.25em] uppercase text-brand-ink-dim reveal reveal-d3">
          <Link
            href="#access"
            className="hover:text-brand-red transition-colors border-b border-transparent hover:border-brand-red pb-0.5 cta-breathe"
          >
            get on the list <span className="hidden md:inline">↗</span>
          </Link>
          <Link
            href="#inquiry"
            className="hover:text-brand-red transition-colors border-b border-transparent hover:border-brand-red pb-0.5"
          >
          ...
          <div className="flex flex-wrap gap-6 mt-4">
            <Link
              href="#events"
              className="text-[10px] text-brand-ink-dim/60 hover:text-brand-ink transition-colors"
            >
              View Events
            </Link>
            <Link
