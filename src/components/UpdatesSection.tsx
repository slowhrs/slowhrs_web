...
export default function UpdatesSection() {
  const feedItems = [
    { title: "ACT III ARCHIVE PROCESSING", date: "05.03.26", category: "RECAP", isLocked: false },
    { title: "NEXT DROP LOADING", date: "12.01.25", category: "UNIFORM", isLocked: true },
    { title: "CASTING WINDOW OPEN", date: "11.20.25", category: "CASTING", isLocked: false },
    { title: "PRIVATE CALENDAR UPDATED", date: "11.15.25", category: "EVENTS", isLocked: true },
    { title: "RUNWAY FILM EXPORTING", date: "11.02.25", category: "FILM", isLocked: false }
  ];

  // Only the first locked card gets the locked.png sticker
  let lockedStickerUsed = false;
        ...
        <div className="flex flex-col gap-4">
          ...
          <div className="flex flex-col border border-brand-border bg-black/20">
            {feedItems.map((item, i) => {
              const isFirstLocked = item.isLocked && !lockedStickerUsed;
              ...
              return (
                <div key={i} className={`flex flex-col sm:flex-row sm:items-center justify-between p-4 md:p-6 border-b border-brand-border/50 hover:bg-[#0a0a0a] transition-colors group cursor-pointer relative overflow-hidden reveal reveal-d${Math.min(i + 1, 5)} ${item.isLocked && !isFirstLocked ? 'backdrop-blur-sm' : ''}`}>
                  ...
                  {/* Blur overlay for non-sticker locked cards */}
                  {item.isLocked && !isFirstLocked && (
                    <div className="absolute inset-0 backdrop-blur-[3px] group-hover:backdrop-blur-[1px] transition-all z-[1] pointer-events-none"></div>
                  )}
                  ...
                  <div className="relative z-10 self-start sm:self-center">
                    <div className={`font-mono text-[10px] tracking-[0.2em] px-2 py-1 uppercase ${item.isLocked ? 'bg-transparent border border-brand-ink/20 text-brand-ink/40' : 'bg-brand-red/10 border border-brand-red/30 text-brand-red'}`}>
                      {item.date}
                    </div>
                  </div>
