...
export default function CinemaEntry({ onComplete }: CinemaEntryProps) {
  ...
  useEffect(() => {
    ...
    // t=3.0s: logo → red, video brighter, "step in" appears
    if (logoRef.current) {
      tl.to(
        logoRef.current,
        {
          filter: "brightness(1) drop-shadow(0 0 40px rgba(230,0,22,0.8))",
          duration: 1,
        },
        3.0
      );
